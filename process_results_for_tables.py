...
def plot_metrics(df_MACRO, suptitle='', macro_range=True):
    stats = []
    ...
    if True:
        ...
        if macro_range:
            ranges = [(0.62, 0.82), (0.24, 0.42), (18, 38), (0.36, 0.56), (0.46, 0.56)]
        else:
            ranges = [(0.62, 0.82), (0.38, 0.48), (14, 24), (0.38, 0.58), (0.42, 0.50)]
        ctr = 0
        for axes, stats in zip(axes_all.flatten(), [stats_er, stats_f, stats_le, stats_lr, stats_seld]):
            ax = axes.bxp(stats, vert=False, patch_artist=True, widths=0.6, showcaps=False, showmeans=False, showfliers=True,
                            manage_ticks=True)
            #axes.set_title(r"$\mathrm{ER}_{\mathrm{LD}}$", fontsize=fs)
            axes.set_title(titles[ctr], fontsize=fs)
            axes.set_xlim(ranges[ctr])
            ctr += 1
            # fill with colors
            ...
            ii = 0
            for box, median in zip(ax['boxes'], ax['medians']):
                box.set_facecolor(cmap(ii))
                median.set(linewidth=5, color=[0.0, 0.0, 0.0])
                ii += 1
            ii = 0
            for jj in range(0, len(ax['whiskers']), 2):
                ax['whiskers'][jj].set(color=cmap(ii))
                ax['whiskers'][jj + 1].set(color=cmap(ii))
                ii += 1
...
if __name__=='__main__':
    ...
    # Median and CIs, using stratified bootstrat across seeds, and wavs
        ...
        df_MACRO = df_MACRO.sort_values('group', ascending=True)
        df_micro = df_micro.sort_values('group', ascending=True)
        #export_individual_csvs(df_MACRO, df_micro)

        suptitle = ''
        suptitle = f'MACRO, {"2022" if params.dcase2022 else "2021"}, ntrials={params.n_trials}'
        plot_metrics(df_MACRO, suptitle=suptitle)

        suptitle = f'micro, {"2022" if params.dcase2022 else "2021"}, ntrials={params.n_trials}'
        plot_metrics(df_micro, suptitle=suptitle)
        ...
        df_MACRO_out = pd.concat(df_MACRO_list, ignore_index=True).sort_values('group', ascending=True)
        df_micro_out = pd.concat(df_micro_list, ignore_index=True).sort_values('group', ascending=True)

        suptitle = ''
        suptitle = f'MACRO, {"2022" if params.dcase2022 else "2021"}, ntrials=1000'
        plot_metrics(df_MACRO_out, suptitle=suptitle, macro_range=True)

        suptitle = f'micro, {"2022" if params.dcase2022 else "2021"}, ntrials=1000'
        plot_metrics(df_micro_out, suptitle=suptitle, macro_range=False)
