...
def test_rotation_OLD():
    ...
    # Now with a lot of random points to find the best ranges for the rotations
    n_points, n_frames, n_rotations = 100, 128, 100
    ...
    # =========================================================================
    # OK PLOT: This looks ok, there are points basically uniformly distributied in the [-1, 1] range, for x,y,z
    sns.displot(data=pd.DataFrame({"data": all_new_points.squeeze().ravel(),
                                   "column": np.repeat(np.arange(all_new_points.shape[1]), all_new_points.shape[0])}),
                x="data", col="column", kde=True, color='blueviolet', height=3)
    plt.show()

    # =========================================================================
    # Now in spherical, 2d histogram
    sns.color_palette("magma", as_cmap=True)
    all_new_points_sph = utils.vecs2dirs(all_new_points.squeeze(), use_elevation=True, include_r=True)
    all_new_points_sph = np.round(all_new_points_sph, decimals=6)  # This is to fix rounding errors with R
    yolo = pd.DataFrame(all_new_points_sph, columns=['azimuth', 'Elevation', 'R'])
    sns.displot(yolo, x="azimuth", y="Elevation", cbar=True, cmap='magma')
    plt.show()

    # Plot with marginals
    g = sns.JointGrid(data=yolo, x="azimuth", y="Elevation")
    g.plot_joint(sns.kdeplot, cmap='magma', kde=True, fill=True)  # Change kdeplot to histplot for bars
    g.plot_marginals(sns.histplot)
    plt.show()

    # Original data
    tmp = utils.vecs2dirs(points, use_elevation=True, include_r=True)
    tmp = np.round(tmp, decimals=6)
    yolo2 = pd.DataFrame(tmp, columns=['azimuth', 'Elevation', 'R'])
    g = sns.JointGrid(data=yolo2, x="azimuth", y="Elevation")
    g.plot_joint(sns.kdeplot, cmap='magma', fill=True)
    g.plot_marginals(sns.histplot)
    plt.show()

    g = sns.PairGrid(yolo2)
    ...
    # =========================================================================
    # Single 2d plot, not sure if this helps
    all_points_together = np.concatenate([all_new_points_sph, tmp], axis=0)
    source = np.concatenate([np.repeat("aug", all_new_points_sph.shape[0]),
                             np.repeat("base", tmp.shape[0])])[:, None]
    daton = np.append(all_points_together, source, 1)
    # daton = np.array(daton, dtype=[('', np.float32), ('', np.float32), ('', np.float32), ('', np.str)])

    # daton = np.array(all_points_together, source, dtype=[('', np.float32), ('', np.float32), ('', np.float32), ('', np.str)] )
    data_tmp = pd.DataFrame(data=daton[9900:, :],
                            # index=[i for i in range(daton.shape[0])],
                            columns=['azimuth', 'elevation', 'r', 'source'])

    data_tmp = data_tmp.astype({'azimuth': np.float32, 'elevation': np.float32, 'r': np.float32, 'source': str})
    # sns.displot(data_tmp, x="azimuth", y="elevation", cbar=True, cmap='magma')
    g = sns.JointGrid(data=data_tmp, x="azimuth", y="elevation", hue='source')
    g.plot_joint(sns.kdeplot, cmap='magma', kde=False, fill=False)  # Change kdeplot to histplot for bars
    g.plot_marginals(sns.histplot)
    plt.show()
    ...
    # =========================================================================
    # This is wrong, I was trying to get side by side plots, with original and augmented data,
    # but I just cannot get it to work
    all_points_together = np.concatenate([all_new_points_sph, tmp], axis=0)
    source = np.concatenate([np.repeat("aug", all_new_points_sph.shape[0]),
                             np.repeat("base", tmp.shape[0])])[:, None]
    daton = np.append(all_points_together, source, 1)
    data_tmp = pd.DataFrame(data=daton[:, :],
                            # index=[i for i in range(daton.shape[0])],
                            columns=['azimuth', 'elevation', 'r', 'source'])
    fig, axes = plt.subplots(1, 2, figsize=(10, 6))
    data_tmp = data_tmp.astype({'azimuth': np.float32, 'elevation': np.float32, 'r': np.float32, 'source': str})
    sns.kdeplot(data_tmp[0:9000], x="azimuth", y="elevation", cbar=True, fill=True, cmap='magma', ax=axes[0])
    # sns.displot(data_tmp, x="azimuth", y="elevation", cbar=True, cmap='magma', col='source', kind='kde')
    # g = sns.JointGrid(data=data_tmp, x="azimuth", y="elevation",   )
    # g.plot_joint(sns.kdeplot, cmap='magma', kde=False, fill=False)  # Change kdeplot to histplot for bars
    # g.plot_marginals(sns.histplot)
    # sns.histplot(data_tmp[10000:], x="azimuth", y="elevation", cbar=True, fill=True, cmap='magma', ax=axes[1])
    # g = sns.JointGrid(data=data_tmp, x="azimuth", y="elevation",ax=axes[1]  )
    # g.plot_joint(sns.kdeplot, cmap='magma', kde=False, fill=False)  # Change kdeplot to histplot for bars
    # g.plot_marginals(sns.histplot)
    plt.show()
    ...
    # =========================================================================
    # GOOOD PLOT
    # this looks good, but needs some work for paper
    g = sns.JointGrid(data=data_tmp.loc[data_tmp['source'] == 'aug'], x="azimuth", y="elevation")
    g.plot_joint(sns.kdeplot, cmap='magma', kde=False, fill=False)  # Change kdeplot to histplot for bars
    g.plot_marginals(sns.histplot, kde=True)
    g.fig.suptitle('Rotated data')
    plt.show()
    ...
    g.plot_marginals(sns.histplot, kde=True)
    g.refline(x=0, y=0)
    g.fig.suptitle('Original data')
    plt.show()

    g = sns.JointGrid(data=data_tmp.loc[data_tmp['source'] == 'aug'], x="azimuth", y="elevation")
    g.plot_joint(plt.hexbin, cmap='magma', gridsize=15)  # Change kdeplot to histplot for bars
    g.plot_marginals(sns.histplot)
    g.fig.suptitle('Rotated data')
    plt.show()
    ...
    # =========================================================================
    # GOOOD PLOT
    # This also looks good, and very usable
    # https://stackoverflow.com/questions/60845764/colorbar-for-sns-jointplot-kde-style-on-the-side
    # https://stackoverflow.com/questions/30975241/seaborn-jointgrid-hexplot-aspect-ratio-issue
    kdeplot = sns.jointplot(data=data_tmp, x='azimuth', y='elevation', kind="kde", fill=True, cmap='magma', cbar=True)
